...
class HAZUSLiquefaction(GDEM):
    ...
    def get_probability_failure(self, sctx, rctx, dctx, gsimtls, epsilons):
        ...
        p_fact = np.zeros_like(kdw)
        for i in range(1, 6):
            idx = sctx.liquefaction_susceptibility == i
            if not np.any(idx):
                continue
            p_fact[idx] = PMU_LIQ[i] / (kmw * kdw[idx])
        p_failure = np.zeros(sctx.liquefaction_susceptibility.shape,
                             dtype=np.float64)
        ...
        for j, epsilon in enumerate(epsilons):
            ...
            p_liq = get_probability_liquefaction(
                pga, sctx.liquefaction_susceptibility)
    ...
    def get_poes(self, sctx, rctx, dctx, imtls, truncation_level):
        ...
        kmw = self._get_magnitude_correction_factor(rctx.mag)
        kdw = self._get_groundwater_depth_correction_factor(sctx)
        # Pre-compute all susceptibility category dependent scalar parameters
        p_fact = np.zeros(sctx.liquefaction_susceptibility.shape,
                          dtype=np.float64)
        ...
        for j, epsilon in enumerate(self.epsilons):
            ...
            if pgdf_latspread in imtls:
                ...
                for i, iml in enumerate(imtls[pgdf_latspread]):
                    ...
                    poes[pgdf_latspread][:, i] += (self.truncnorm_probs[j] *
                                                   p_failure *
                                                   (displacement >= iml))
    ...
    def get_displacement_field(self, rupture, sitecol, cmaker, num_events=1,
        ...
        kmw = self._get_magnitude_correction_factor(gmf_computer.ctx[1].mag)
        kdw = self._get_groundwater_depth_correction_factor(
            gmf_computer.ctx[0])
        ...
        for j in range(p_failure.shape[1]):
            p_failure[:, j] = p_fact * get_probability_liquefaction(pga[:, j],
                gmf_computer.ctx[0].liquefaction_susceptibility) 
...
class HAZUSLandsliding(GDEM):
    ...
    def get_probability_failure(self, sctx, rctx, dctx, gsimtls=None):
        ...
        for j, epsilon in enumerate(self.epsilons):
            ...
            if np.any(idx):
                p_failure[idx] += (self.truncnorm_probs[j] * pma[idx])
        return p_failure
    ...
    def get_poes(self, sctx, rctx, dctx, imtls, truncation_level):
        ...
        for j, epsilon in enumerate(self.epsilons):
            ...
            e_d_ub, e_d_lb = self._get_expected_displacement_factor(a_c[idx] /
                                                                    pga[idx])

            lb_displacement = M_PER_INCH * n_cycles * e_d_lb * pga[idx]
            ub_displacement = M_PER_INCH * n_cycles * e_d_ub * pga[idx]
    ...
    def get_displacement_field(self, rupture, sitecol, cmaker, num_events=1,
        ...
        gmfs = gmf_computer.compute(self.gmpe_set, num_events, seed=None)
        # Get the PGA field
        pga = np.exp(gmfs[list(self.gmpe_set).index("PGA")])
        # Return the critical acceleration and proportion of mapped area
        a_c, pma = self._get_ac_map_area(gmf_computer.ctx[0])
        ...
        n_cycles = self._get_number_cycles(gmf_computer.ctx[1].mag)
        # Calculate ratio of acceleration to critical acceleration
        a_c_ais = a_c[mask] / pga[mask]
